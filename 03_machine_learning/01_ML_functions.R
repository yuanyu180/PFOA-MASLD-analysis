...
ExtractVar <- function(fit){
  Feature <- quiet(switch(
    ...
    "rfsrc" = {
      ...
      if (!is.null(imp) && length(imp) > 0) {
        selected_vars <- names(imp)[imp > 0]
        if (length(selected_vars) == 0) {
          imp_sorted <- sort(imp, decreasing = TRUE)
          selected_vars <- names(imp_sorted)[1:min(10, length(imp_sorted))]
        }
        selected_vars
      } else {
        fit$subFeature
  ...
  if (length(Feature) == 0) {
    warning("No features selected, using all features as fallback")
    Feature <- fit$subFeature
  }
  return(Feature)
}
